...
TEST_CASE("GetCalibrationTest")
{
    ...
    const auto checkTransform = [](const CRTProtocol::SCalibrationTransform& l,
                                   const CRTProtocol::SCalibrationTransform& r) -> void
    {
        CHECK_EQ(l.x, r.x);
        CHECK_EQ(l.y, r.y);
        CHECK_EQ(l.z, r.z);
    ...
    const auto checkIntrinsic = [](const CRTProtocol::SCalibrationIntrinsic& l,
                                   const CRTProtocol::SCalibrationIntrinsic& r) -> void
    {
    ...
    for (std::size_t i = 0; i < expectedCalibration.cameras.size(); ++i)
    {
        ...
        CHECK_EQ(expectedCamera.active, actualCamera.active);
        CHECK_EQ(expectedCamera.calibrated, actualCamera.calibrated);
        CHECK_EQ(expectedCamera.message, actualCamera.message);
        CHECK_EQ(expectedCamera.point_count, actualCamera.point_count);
        CHECK_EQ(expectedCamera.avg_residual, actualCamera.avg_residual);
